					...
					if(isset($_GET['u']) && $_GET['u']) {
						...
						$t = getcookie('t');
							...
							if($u>0) dsetcookie('t',$u,3600*24*30);
										...
										$time = TIMESTAMP - 86400 * 5;
										DB::delete('yaoqing',"t_uid = $u AND uid = 0  AND dateline < ".$time);
										$arr = array();
										$arr['t_uid'] =  $u;
										...
										if($useragent){
											...
											//微信端 不记录
										if($platform == 4 && $_GET['m'] != 'tui') return ;
	...
	static  function run(){
					...
					if(defined('IN_ADMIN') && $_G[uid]  &&  $_G[member][groupid] != 1){
						...
						if($a == 'del' && $power[$m]['post'] ==1 ){
							...
							if($power[$m][$a] && $power[$m][$a] !=1 )  cpmsg('当前模块分类您无法进行操作','errr',$jump_url);
						}
					}
