...
class apps extends app{
		...
		function count(){
			...
			if($_G['setting']['apicloud_appid'] && $_G['setting']['apicloud_appkey'] && !preg_match("^A{12,20}$/",$_G['setting']['apicloud_appid'] )){
		...
		function xiaohdp(){
				...
				if($_GET['onsubmit'] && check() ){
					...
					$arr = serialize($arr);
					if(isset($_G['setting']['xiaohdp'])){
						set_setting('xiaohdp',$arr );
					}else{
						insert_setting('xiaohdp',$arr );
					}
					loadcache('setting','update');
					cpmsg('修改成功','success','m=apps&a=xiaohdp');
					return false;
				}

			
			if($_G[setting]['xiaohdp']){
				$hdp = dunserialize($_G[setting]['xiaohdp']);
			}else{
				$hdp = array(array('picurl'=>'','url'=>''));
			}
		...
		function xiaogezi(){
				...
				if($_GET['onsubmit'] && check() ){
					...
					$arr = serialize($arr);
					if(isset($_G['setting']['xiaogezi'])){
						set_setting('xiaogezi',$arr );
					}else{
						insert_setting('xiaogezi',$arr );
					}
					loadcache('setting','update');
					cpmsg('修改成功','success','m=apps&a=xiaogezi');
					return false;
				}

			
			if($_G[setting]['xiaogezi']){
				$hdp = dunserialize($_G[setting]['xiaogezi']);
			}else{
				$hdp = array(array('picurl'=>'','url'=>''));
			}
			$size = 6;
