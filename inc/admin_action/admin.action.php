...
class admin extends app{
		...
		function caiji(){
				...
				if($_GET['onsubmit'] && check() ){
					...
					if(!empty($_GET['tk_data'])){
						$tk_data = json_decode($_GET['tk_data'],1);
						if(empty($tk_data) || !isset($tk_data['access_token'])) msg('高佣信息格式化失败,请检查是否填写完整');
						$time = intval($tk_data['expire_time'] / 1000);
						$time = date('Y-m-d H:i',$time);
						$token = $tk_data['access_token'];
						if(!isset($_G['setting']['tk_token'])){
							insert_setting('tk_token',$token );
							insert_setting('tk_token_time',$time );
						}else{
							set_setting('tk_token',$token);
							set_setting('tk_token_time',$time);
						}
					}
