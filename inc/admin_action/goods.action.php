...
class goods extends app{
		...
		function quan_setting(){
			...
			$dataoke = array(1=>'女装',2=>'母婴',3=>'化妆品',4=>'居家',
				5=>'鞋包配饰',6=>'美食',7=>'文体车品',8=>'数码家电',9=>'男装',10=>'内衣');

			$is_extends = file_exists(ROOT_PATH.'dtk.php');
			if(!$is_extends) $_G['setting']['dtk_jump'] = 0 ;
			
			$this->add(array('dataoke'=>$dataoke,'is_extends'=>$is_extends));
		...
		//好品推接口
		function hpt_setting(){
			global $_G;

			//$dataoke = array(1=>'服装',2=>'母婴',3=>'化妆品',4=>'居家',5=>'鞋包配饰',6=>'美食',7=>'文体车品',8=>'数码家电');
			$list = array(1=>'女装',2=>'男装',3=>'鞋包',4=>'配饰',5=>'户外',6=>'美妆',
				7=>'母婴',8=>'食品',9=>'内衣',10=>'数码',11=>'家装',12=>'居家',13=>'家电',
				14=>'汽车',15=>'服务',16=>'图书',17=>'其它');
			$this->add(array('list'=>$list));

			if($_GET['onsubmit'] && check()){
				$arr =serialize($_GET['web_cate']);
				insert_setting('hpt_cate',$arr);
			}
			parent::seo_setting();
		}
	...
	public function update_hpt(){
			global $_G;
			if ($_G['setting']['hpt_appkey'] ) {
				$haopintui = top('haopintui');
				$haopintui->auto_update();
			}else{
				msg('好品推appkey未设置');
			}
	}
...
function quan_import(){
	...
	$rs = DB::fetch_all("SELECT juan_url,aid,title,juan_price,yh_price,title,picurl,url,num_iid,bili_type FROM ".DB::table('goods').
				...
				foreach($rs as $k=>$v){
					...
					$url = $v['juan_url'];
					if(!$url) $url = $v['url'];
					$url = creat_url($url,$v['num_iid'],$v['bili_type'] );
