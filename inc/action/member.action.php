...
class member extends app{
	...
	function reg(){
		...
		/**
		 * [tui_start description]
		 * @param  integer $t_uid       推荐者的UID
		 * @param  integer $lv          几级代理 
		 * @param  array   $curent_user 当前注册者 或是 上一级的
		 * @return [type]               [description]
		 */
private	 	 function tui_start($t_uid = 0,$lv = 3,$curent_user = array()){
	 		global $_G;
	 		if($lv < 1 ) return ;

	 		//推荐者信息
			$tmp =DB::fetch_first("SELECT * FROM ".DB::table('member')." WHERE uid ='$t_uid' ");
			if($tmp[uid]>0 && $tmp[groupid] != 3 && $tmp[check] ==1){
				$update = array();
				
				
				$jf = $_G['setting']['daili_'.$lv];
				if(empty($jf)) {
					if($lv == 3 && $_G['setting']['yaoqing'] > 0  ) {
				...
				$desc = '邀请 '.$curent_user['username'].' 注册';

				if($lv < 3 ) $desc = '您的下级 '.$curent_user['username'].' 邀请新用户注册';
				$add_jf = 	$tmp['jf']+$jf;
				$sid =insert_sign(array('uid'=>$tmp[uid],'username'=>$tmp[username],'desc'=>$desc,
							'type'=>'yaoqing','type_id'=>$t_uid,'org_jf'=>$add_jf,'jf'=>$jf));

				if($sid){
					 $update['jf'] = $tmp[jf]+$jf;
					 
				}
				if(count($update)>0)	update_member(array('jf'=>$tmp[jf]+$jf),$tmp[uid]);

				if($tmp['t_uid'] > 0 && $lv > 1 ) $this->tui_start($tmp['t_uid'],$lv-1,$tmp);
			}
	 }
