...
class ajax extends app{
	...
	/**
	 * 是否可增加积分
	 * @param  string  $type    favorite或like
	 * @param  integer $day  1或30
	 * @return boolean       [description]
	 */
	function is_add_favrorite($type='goods'){
		global $_G;
		if($_G['setting']['addfavorite_jf']>0){

				$and = " uid = ".$_G['uid'] ." AND type ='".$type."'";
				$max = intval($_G['setting']['addfavorite_day']);
				if($max >0){
						$count_day = getcount('favorite',$and." AND dateline >".TODAY);
						if($count_day>= $max) return false;
				}
			
				$max30 = intval($_G['setting']['addfavorite_num']);
				if($max30 >0){
						$day30 = strtotime('-30 day');
						$count_day30 = getcount('favorite',$and." AND dateline >".$day30);
						if($count_day30>= $max30) return false;
				}
		}
		return true;
	}
	...
	function favorite(){
		...
		if(!$rs ||  !$rs[id]){
					...
					if($this->is_add_favrorite($type) && $jf>0) {
							$msg=",系统扣除你".$jf."个积分";
							$dec_jf = 0-$rs['jf'];
	...
	function get_link(){
		if(!isset($_GET['num_iid'])) json('num_iid不能为空');
		$activity_id = isset($_GET['activity_id']) ?  $_GET['activity_id'] :'';
		$url = top('link','get',$_GET['num_iid'],$activity_id);
		if(strpos($url,'http') === false && !isset($_GET['debug'])) {
			json($url);
		}
