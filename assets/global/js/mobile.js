...
function open_url(url,open,app_data){
		...
		if(url[0] == '/' || url[0] == '?' || url.indexOf(location.host) >-1) {
			...
			}else if(url.indexOf('dtk.php') > -1){
				url = '/assets/common/jump.html?jumpurl='+encodeURIComponent(location.origin +url);
			}else{
				open = false;
			}
		...
		if(!is_weixin && url.indexOf('dtk_id') > -1 ){
			var dtk_id = sub_str(url+'&' ,'dtk_id=','&');
           	url = '/dtk.php?r=index/middleurl&id='+dtk_id;
		}
		if(open){
			window.open(url)
		}else{
...
function jump(href,call){
		//走高佣
		//var init  = $(el).attr('init');
		//if(init == 1 ) {}
		if(href && href.indexOf('uland.taobao.com') > -1 ) {
				if(href.indexOf('edetail?e=') > -1 ) return true;
			var param = getUrlParam(href);
			if(param.itemId) {
				var activityId = param.activityId;
				var url = '/?m=ajax&a=get_link&num_iid='+param.itemId;
				if(activityId) url+='&activity_id='+activityId;
				// var newWin = window.open('about:blank');
				ajaxget(url,function(rs){
					//$(el).attr('init',1);
					if(rs.status == 'success' && typeof rs.data == 'string' && rs.data.indexOf('http') > -1 ) {
						//$(el).attr('href',rs.data);
						call && call (rs.data);
						//newWin.location.href= rs.data;
						//window.open(rs.data);
					}else{
						call && call (href);
