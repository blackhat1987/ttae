...
//								rs+='<input type="hidden" name="dp_new[]"  value=""/></td></tr>';
	...
	,apps_hdp:function(title){
		...
		this.apps_hdp('格子');
	},apps_xiaohdp:function(){
			this.apps_hdp('小程序幻灯片');
	},apps_xiaogezi:function(){
		this.apps_hdp('小程序格子');
	}
	,apps_tpl:function(){
	...
	fetch_lianmeng_list:function(){

			$('.table_main .select_fid').change(function(){
				var a = $(this).closest('tr').find('a.red');
				var id = this.value;
				var field = $(this).attr('field');
				a.attr(field,id);
				var reg = new RegExp("&"+field+"=(\d){1,5}$",'i');
				var url = a.attr('href').replace("&fid=0",'');
				url = url.replace('&flag=0','');
				url = url.replace(reg,'');
				if(id==0){
					a.attr('href' ,url);
					return ;
				}
				a.attr('href' ,url+'&'+field+'='+id);
			})
		...
		$('form').eq(1).submit(function(){
			...
			$('.kg_editorContainer img').each(function(i){
				var src = $(this).attr('src');
				if(src.indexOf('qpic.cn')  ==  -1 ){
					pic_len++;
					is_post = true;
					
					download_img(src,'content',i);
				}
			});

			var picurl = $('.pic_upload').val();
			if(picurl.indexOf('qpic.cn')  ==  -1 && picurl.indexOf('/')  > -1){
				pic_len++;
				download_img(picurl,'thumb',-1);
			}
			if(pic_len == 0) return true;
			Dialog.info(pic_len+'张图片正在上传到微信服务器...','success',60000);
			return false;
		})

		function ajaxpost(data,success,error){
			$.ajax({
				url: admin_url+'?m=weixin&a=download',
				type: 'POST',
				dataType: 'JSON',
				data:data,
			})
		...
		function download_img(url,type,index){
			var pic = encodeURIComponent(url);
			var data = {'pic':pic,type:type};
			ajaxpost(data,function(rs){
				if(rs.status == 'success'){
					pic_len -- ;
					if(pic_len<=0) {
						is_over = true;
						$('.kg_editorContainer img').eq(0).click();
						Dialog.info('文章内所有图片上传到微信完成,2秒后自动提交','success',4000,function(){
							// $('form').eq(1).submit();
							$('.submit_btn').click();
						});
					}
					 ...
					 is_post = false;
					is_over = true;
					showDialog('图片下载失败:'+rs.msg);
				}
			},function(rs){
				 is_post = false;
				is_over = true;
				showDialog('图片下载失败,请查看错误日志'+url);
			});
		}
