...
<div class="item_list" >
    <a class="imgd _open" data-url="{if $v.juan_url}{$v.juan_url}{else}{$v.url}{/if}"  {if $_G.setting.dtk_jump}data-dtk_id="{$v.dtk_id}"{/if}>
        <img width="140" src="{$v.picurl}_250x250.jpg">
        {if $v.new ==1}<i class="mb_ico goodsdpi gisnew1"></i>{/if}
        <span class="goodsisover1"></span>
    </a>
    <h2><span><a class="_open"  data-url="{if $v.juan_url}{$v.juan_url}{else}{$v.url}{/if}"  {if $_G.setting.dtk_jump}data-dtk_id="{$v.dtk_id}"{/if}>{$v.title}</a></span></h2>
    <aside>
 {if $v.juan_url}
<a data-url="{$v.juan_url}"  rel="nofollow"  class="y juan_btn _open"  {if $_G.setting.dtk_jump}data-dtk_id="{$v.dtk_id}"{/if}>领{if $v.juan_price}{$v.juan_price}元{/if}优惠券</a>
{/if}
