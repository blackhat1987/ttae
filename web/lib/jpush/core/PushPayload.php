...
class PushPayload {
    ...
    public function addAndroidNotification($alert=null, $title=null, $builderId=null, $extras=null,$picurl=null) {
        $android = array();
       ...
       //todo 自己加的,没办法继承这个类.只能改这个文件了
        if (!empty($picurl)) {
            if(!is_string($picurl)) {
                throw new InvalidArgumentException("Invalid android picurl value");
            }
            if(strlen($picurl) > 0) {
                $android['big_pic_path'] = $picurl;
                $android['style'] = 3;
            }
        }
